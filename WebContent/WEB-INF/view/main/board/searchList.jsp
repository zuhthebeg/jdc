    ...
    <div class="container">
		...
		<ul class="nav nav-pills nav-stacked" style="">
			<c:if test="${empty questionList}">
				<li>등록한 질문이 존재하지 않습니다!</li>
			</c:if>
			<c:forEach items="${questionList}" var="article" varStatus="i">
				<li>
		...
		<ul class="nav nav-pills nav-stacked" style="">
			<c:if test="${empty answerList}">
				<li>등록한 답변이 존재하지 않습니다!</li>
			</c:if>
			<c:forEach items="${answerList}" var="article" varStatus="i">
				<li>
					<a href="/JDC/board/article/${article.article_idx}" >[ RE : ${article.code_name} ] ${article.title} 
						<span style="float: right;">${article.regdate}</span>
						<span style="float: right; margin-right: 100px;"> ${article.reply_cnt} 개의 댓글이 달렸습니다. </span>
						<span style="float: right;"> ${article.answer_cnt} 개의 답변, </span>
					</a>
